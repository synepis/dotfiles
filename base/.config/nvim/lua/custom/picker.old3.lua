...
function Async.scheduler()
	local co = coroutine.running()
	assert(co, "Scheduler must be called inside an async function")
...
local sleep = Async.await(function(ms, callback)
	vim.defer_fn(callback, ms)
end)

-- Define our async block
local run_test = Async.run(function()
...
local function update_selection(selected_idx)
	if selected_idx <= #state.results then
		state.selected_idx = selected_idx
	else
		state.selected_idx = nil
	end

	-- Always clear out previous mark
	...
	if selected_idx then
		if state.results[selected_idx] then
			debug_log("Highlighting " .. selected_idx)
...
local function render_preview()
	...
	elseif selection.bufnr then
		state.preview_buf = selection.bufnr
...
local function render_results(results, opts)
	-- We are modifying the vim UI so we should ensure we are on the main UI thread
	vim.schedule(function()
		...
		if vim.api.nvim_buf_is_valid(state.results_buf) then
			...
			for line_idx, line_hls in ipairs(highlights) do
				-- local virt_line = { { "fsdfdsf" } }
				-- vim.api.nvim_buf_set_extmark(state.results_buf, state.namespace, line_idx - 1, 0, {
...
local pipelines = {
	...
	diagnostics = {
		find = function(_, callback)
			...
			for _, diag in ipairs(diagnostics) do
				...
				local label = string.format("[%d] [%s] %s", diag.bufnr, severity, diag.message:gsub("\n", " "))
				table.insert(options, {
					key = #options,
					label = { text = label },
					bufnr = diag.bufnr,
				...
				local function process_symbols(symbols, container_name)
					for _, symbol in ipairs(symbols) do
						...
						if kind_name == "Function" or kind_name == "Method" then
							local display_name = container_name and (container_name .. " > " .. symbol.name)
								or symbol.name
							table.insert(options, {
								key = #options,
	...
	files = {
		...
		filter = function(_, query, callback)
			...
			-- also `filter_options` must be on the main thread
			vim.schedule(function()
				local filtered = filter_options(query)
				callback(filtered)
			end)
	...
	live_grep = {
		...
		filter = function(options, _query, callback)
...
local function focus_first_result_line()
...
local function accept_selection()
	...
	if selected and selected.filename then
		debug_log("Editing filename: " .. vim.fn.fnameescape(selected.filename))
	...
	elseif selected and selected.bufnr then
		debug_log("Selecting bufnr: " .. selected.bufnr)
		debug_log("Orig win" .. state.original_win)
...
local function setup_keybindings()
	...
	vim.keymap.set("n", "j", focus_first_result_line, { buffer = state.input_buf })
	...
	vim.keymap.set("n", "<CR>", "<NOP>", { buffer = state.input_buf })
...
local function setup_event_listeners()
	...
	vim.api.nvim_create_autocmd({ "CursorMoved" }, {
		...
		callback = function()
			state.debounce_timer:stop()
			state.debounce_timer:start(20, 0, vim.schedule_wrap(render_preview))
		end,
	})
	...
	local plugin_windows = {
		[state.input_win] = true,
		[state.results_win] = true,
		[state.preview_win] = true,
	}

	-- TODO: Remove this autogroup
	local group_id = vim.api.nvim_create_augroup("PickerWindowLock", { clear = true })

	vim.api.nvim_create_autocmd("WinEnter", {
		group = group_id,
		callback = function()
			local current_win = vim.api.nvim_get_current_win()
			if not plugin_windows[current_win] then
				for valid_win, _ in pairs(plugin_windows) do
					if vim.api.nvim_win_is_valid(valid_win) then
