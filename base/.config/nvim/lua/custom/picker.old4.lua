...
local function init_windows(mode, input_position, preview_position)
	...
	state.preview_scratch_buf = vim.api.nvim_create_buf(false, true)
	state.preview_buf = state.preview_scratch_buf
...
local function format_option(option, opts)
	...
	if opts.show_row and option.row then
		...
		-- append_chunk(string.format("%3d", option.row), "PickerNumber")
	end

	if opts.show_col and option.col then
		append_chunk("|", "PickerDelimiter")
		append_chunk(tostring(option.col), "PickerNumber")
		-- append_chunk(string.format("%3d", option.col), "PickerNumber")
	end
...
local function render_preview()
	...
	-- Reset our scratch buffer if it has been changed
	state.preview_buf = state.preview_scratch_buf
	...
	elseif selection.bufnr then
		vim.api.nvim_win_set_buf(state.preview_win, selection.bufnr)
		vim.api.nvim_win_set_cursor(state.preview_win, { selection.row or 1, selection.col or 0 })
	end
end
	...
	symbols = {
		find = function(_query, callback)
			...
			vim.lsp.buf_request(bufnr, method, params, function(err, result, ctx, config)
				...
				debug_log(vim.inspect(symbol_kind_map))
				...
				local function process_symbols(symbols, depth)
					for _, symbol in ipairs(symbols) do
						...
						local display_name = string.format("%s %s [%s]", indent, symbol.name, kind_name)
...
local function setup_event_listeners()
	...
	vim.api.nvim_create_autocmd("WinLeave", {
		...
		callback = function()
			...
			if vim.api.nvim_get_current_win() == state.results_win then
				vim.bo[state.results_buf].modifiable = true
			elseif vim.api.nvim_get_current_win() == state.preview_win then
